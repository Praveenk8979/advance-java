...
<script type="text/javascript">
	...
	function viewFile(fileNo, sno, right, contractorName) {
		currentSno = sno;
		currentAlloteeName = contractorName

		if (right == 1) {
			var url = "viewFile?id=" + fileNo + "&sno=" + sno
					+ "&department=HR&prFlage=null&name=" + contractorName;
	...
	function getPage(type, page) {
		...
		if (type == 'noteDiv' || type == 'notePage') {
			currentNote = page;
			div.innerHTML = '<object oncontextmenu="return false" style="height: 100%; width: 100%;" data="staticResources/pdfs/'
					+ fileId + 'L@' + page + 'L.pdf#toolbar=0"></object>';
		} else {
			currentCorr = page;
			div.innerHTML = '<object oncontextmenu="return false" style="height: 100%; width: 100%;" data="staticResources/pdfs/'
					+ fileId + 'R@' + page + 'R.pdf#toolbar=0"></object>';
		}
	}
	...
	function setPageList() {
		...
		for (var i = 0; i < noteCount; i++) {
			if (i == 0)
				...
				notePage.innerHTML = notePage.innerHTML
		...
		for (var i = 0; i < corrCount; i++) {
			if (i == 0)
				...
				corrPage.innerHTML = corrPage.innerHTML
						+ '<option value="'+i+'">' + i + '</option>'
		}
	}
	function downloadFile(fileCode, sno, right) {
		if (right == 1)
			window.location = "downloadFile?id=" + fileCode + "&sno=" + sno
					+ "&department=HR";
	}
	...
	function printOut(fileId, currentSno, printLr, currentAlloteeName) {
		var url = "viewFile?id=" + fileId + "&sno=" + currentSno
				+ "&department=HR&prFlage=print&name=" + currentAlloteeName
				+ "&lr=" + printLr;
	...
	<table style="width: 100%;">
		<tr>
			<td>
				...
				<select style="width: 70px; height: 25px;" id="notePage"
			...
			<td>
				...
				<select style="width: 70px; height: 25px;" id="corrPage"
...
<div id="printModel" class="base-modal"
	...
	<a href="#"
		...
		onclick="deleteFile();"
		class="base-closebtn base-hover-red base-display-topright">X</span></a>
	<div id="printDiv"
		class="base-modal-content base-card-8 base-animate-zoom"
...
<div id="printConfModal" class="modal"
	style="display: none; z-index: 1000000;">
	<div class="modal-content" style="width: 20%;">
		<div class="modal-header" style="background-color: #387403;">
			<span class="close"
				onclick="document.getElementById('printConfModal').style.display='none'"
				style="color: #FFFFFF;">&times;</span>
			<table style="width: 100%;">
				<tr>
					<td colspan="2" align="center"><p
							style="text-align: center; color: #FFFFFF; margin-top: 0px; padding-top: 0px;"
							class="h3" id="printPara">Noting Print</p></td>
				</tr>
				<tr>
					<td align="center"><button
							class="base-button base-round-large"
							style="background-color: #ffffff;" onclick="printFile('single')">Current
							Page</button></td>
					<td align="center"><button
							class="base-button base-round-large"
							style="background-color: #ffffff;" onclick="printFile('cust')">Customize</button></td>
				</tr>
			</table>
		</div>
	</div>
...
<p class="h1"
	style="font-family: cambria; text-align: center; color: #387403;">HR Retrieve</p>
<div style="margin-bottom: 0px; padding-bottom: 0px; margin-left: 1%;">
	<HrForm:form action="retrieveHR" id="HRForm" method="get"
		modelAttribute="HRForm">
		<table
			style="border-spacing: 20px; border-top: 0px; border-collapse: separate;">
			<tr>

				<td><label style="font-family: cambria;" for="Contractor Name"><h4>
							<b>File No:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						id="fileNo" path="fileNo" list="contractorNameHelp" /> <datalist
						id="contractorNameHelp"></datalist></td>
				<td><label style="font-family: cambria;" for="Work Name"><h4>
							<b>File Code:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						id="fileCode" path="fileCode" list="workHelp" /> <datalist
						id="workNameHelp"></datalist></td>
				<td><label style="font-family: cambria;" for="OPA/FTS"><h4>
							<b>File Subject.:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="fileSubject" id="file_Subject" list="opaFtsHelp" /> <datalist
						id="opaFtsHelp"></datalist></td>
				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>OPA/FTS:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="opaFts" id="opaFts" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>
			...
			<tr>
				...
				<td><label style="font-family: cambria;" for="Category"><h4>
						...
						path="year" id="year" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Employee Category:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="employeeCategory" id="employeeCategory" list="categoryHelp" />
					<datalist id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Date:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="date" id="date" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Employee Code:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="employeeCode" id="employeeCode" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>
			</tr>
			...
			<tr>
				...
				<td><label style="font-family: cambria;" for="Category"><h4>
						...
						path="employeeName" id="employeeName" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Employee Designation:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="employeeDesignation" id="employeeDesignation"
						list="categoryHelp" /> <datalist id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Employee DOB:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="employeeDob" id="employeeDob" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>

				<td><label style="font-family: cambria;" for="Category"><h4>
							<b>Employee DOJ:</b>
						</h4></label><br> <HrForm:input style="width: 230px; height: 35px;"
						path="employeeDoj" id="employeeDoj" list="categoryHelp" /> <datalist
						id="categoryHelp"></datalist></td>
				...
				<c:forEach items="${records}" var="record">
					<tr>
						...
						<td><a href="#"
							...
							style="text-decoration: none;">${record.fileNo}</a></td>
						<td>${record.fileCode}</td>
						<td>${record.fileSubject}</td>
						<td>${record.opaFts}</td>
						<td>${record.totalPages}</td>
						<td><a href="#"
							onclick="viewFile('${record.fileNo}','${record.sno}','${view}','${record.fileSubject}')"
							style="text-decoration: none;">View</a>&nbsp;&nbsp; <c:if
								test="${download=='1'}">
								<a href="#"
									onclick="downloadFile('${record.fileNo}','${record.sno}','${download}');"
									style="text-decoration: none;">Download</a>&nbsp;&nbsp;</c:if> <c:if
								test="${print=='1'}">
								<a href="#"
									onclick="printOut('${record.fileNo}','${record.sno}','${print}','${record.fileCode}');"
