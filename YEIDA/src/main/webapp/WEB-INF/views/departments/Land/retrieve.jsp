...
currentSno=sno;currentAlloteeName=name;
	...
	function renderPage(num) {
	  pdfDoc.getPage(num).then(function (page) {
	    ...
	    document.getElementById('noteDiv').innerHTML = ''; // Clear previous content
	    document.getElementById('noteDiv').appendChild(canvas);
	  });
	}

	// Rotate function
	function rotate(divId, direction) {
	  if (direction === 'L') {
	    rotation -= 90; // Rotate left
	  } else if (direction === 'R') {
	    rotation += 90; // Rotate right
	  }
	  rotation = rotation % 360; // Keep the rotation within 0-360 degrees
	  renderPage(pageNum); // Re-render the page with the new rotation
	}

	// Load the PDF document when the page loads
	window.onload = function () {
	  loadPDF('path-to-your-pdf.pdf'); // Replace with the path to your PDF
	};
</script>
	...
	<table style="width: 100%;">
		<tr>
			...
			<td>
                ...
                <button class="btn btn-primary" id="rotatel" onclick="rotate('noteDiv', 'L');">Rotate Left</button>
			...
			<td>
				...
				<button class="btn btn-primary" id="preButNote" onclick="firLas('corrDiv','las');">Last</button>
   				<button class="btn btn-primary" id="rotater" onclick="rotate('corrDiv', 'R');">Rotate Right</button>
