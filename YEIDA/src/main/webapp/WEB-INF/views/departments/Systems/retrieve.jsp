...
<script type="text/javascript">
	...
	function retrieveFiles()
	{
		...
		var optFts=document.getElementById('optFts').value;
		var SubjectName=document.getElementById('subjectName').value;
		//var No_Of_NoteSheet=document.getElementById('no_Of_NoteSheet').value;
		if(lotNo=='' && clerk_Name=='' &&  file_No=='' && Department=='' && optFts=='' && SubjectName=='' 
		//	 && No_Of_NoteSheet==''
				 )
	...
	function deleteFile()
	{
		window.location="delView?department=Systems";
	}
	function viewFile(pos,sno,right,name)
	{
		...
		if(right==1)
		{
			var url="viewFile?id="+pos+"&sno="+sno+"&department=Systems&prFlage=null&name="+name;
	...
	{
		if(right==1)
			window.location="downloadFile?id="+pos+"&sno="+sno+"&department=Systems";
	}
	function updateFile(sno,right)
	{
		if(right==1)
			window.location="updateFile?sno="+sno+"&department=Systems";
	}
	...
	function printOut(fileId,currentSno,printLr,currentAlloteeName)
	{
			...
			var url="viewFile?id="+fileId+"&sno="+currentSno+"&department=Systems&prFlage=print&name="+currentAlloteeName+"&lr="+printLr;
...
<p class="h1" style="font-family: cambria; text-align: center; color: #387403;">System</p>
<div style="margin-bottom: 0px; padding-bottom: 0px; margin-left: 1%;">
    <SystemForm:form action="retrieveSystem" id="SystemForm" method="get" modelAttribute="SystemForm">
        <table style="border-spacing: 20px; border-top:0px; border-collapse: separate;">
            <tr>
            	<td>
                	<label style="font-family: cambria;" for="Court Name"><h4><b>Lot No:</b></h4></label><br>
                	<SystemForm:input style="width: 230px; height: 35px;" path="lotNo" id="lotNo" list="courtNameHelp" onkeyup="getHelp('courtName');"/>
                	<datalist id="courtNameHelp"></datalist>
                </td>
            	<td>
            		<label style="font-family: cambria;" for="PetitionNo"><h4><b>Clerk Name:</b></h4></label><br>
            		<SystemForm:input style="width: 230px; height: 35px;" id="clerk_Name" path="clerk_Name" list="petitionNoHelp" onkeyup="getHelp('petitionNo');"/>
            		<datalist id="petitionNoHelp"></datalist>
            	</td>
                <td>
                	<label style="font-family: cambria;" for="Party Name"><h4><b>File No:</b></h4></label><br>
                	<SystemForm:input style="width: 230px; height: 35px;" id="file_No" path="file_No" list="partyNameHelp" onkeyup="getHelp('partyName');"/>
                	<datalist id="partyNameHelp"></datalist>
                </td>
                <td>
                	<label style="font-family: cambria;" for="Advocate Name"><h4><b>Department:</b></h4></label><br>
                	<SystemForm:input style="width: 230px; height: 35px;" path="department" id="department" list="advocateNameHelp" onkeyup="getHelp('advocateName');"/>
                ...
                <td>
                	...
                	<SystemForm:input style="width: 230px; height: 35px;" path="optFts" id="optFts" list="respondentAdvocateHelp" onkeyup="getHelp('respondentAdvocate');"/>
                	<datalist id="respondentAdvocateHelp"></datalist>
                </td>
            </tr>
            <tr>
            	<td>
            		<label style="font-family: cambria;" for="Year"><h4><b>Subject Name:</b></h4></label><br>
            		<SystemForm:input style="width: 230px; height: 35px;" id="subjectName" path="subjectName" list="yearHelp" onkeyup="getHelp('year');"/>
            		<datalist id="yearHelp"></datalist>
            	</td>
            	<%-- <td>
            		<label style="font-family: cambria;" for="Department"><h4><b>No_Of_Note_Sheet:</b></h4></label><br>
            		<SystemForm:input style="width: 230px; height: 35px;" id="no_Of_NoteSheet" path="no_Of_NoteSheet" list="relatedDepartmentHelp" onkeyup="getHelp('relatedDepartment');"/>
...
<c:if test="${not empty records}">
	<form action="generateReport" id="reportForm" method="post">
		<input type="hidden" name="department" value="Systems">
			...
			<tbody>
				<c:forEach items="${records}" var="record">
					<tr>
						...
						<td>${record.sno}</td> 					
						<td><a href="#" onclick="updateFile('${record.sno}','${update}')" style="text-decoration: none;">${record.file_No}</a></td>
       ...
       <td>            
							<a href="#" onclick="viewFile('${record.file_No}','${record.sno}','${view}','${record.file_No}')" style="text-decoration: none;">View</a>&nbsp;&nbsp;
							<c:if test="${download=='1'}"><a href="#" onclick="downloadFile('${record.file_No}','${record.sno}','${download}');" style="text-decoration: none;">Download</a>&nbsp;&nbsp;</c:if>
							<c:if test="${print=='1'}"><a href="#" onclick="printOut('${record.file_No}','${record.sno}','${print}','${record.clerk_Name}');" style="text-decoration: none;">Print</a></c:if>
						</td>
					</tr>
				</c:forEach>
			</tbody>
