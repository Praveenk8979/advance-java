...
public class EMLController
{
	...
	public String updateLaw(HttpServletRequest request,@ModelAttribute("lawForm")Law law,@RequestParam("noteSheet")MultipartFile noteSheet,@RequestParam("correspondence")MultipartFile correspondence,RedirectAttributes flashAttributes)throws IOException
	{
		...
		if(noteSheet!=null && noteSheet.getOriginalFilename().trim().length()>0)
		{
			if(!noteSheet.getOriginalFilename().equals(law.getWritPatitionNumber()+"L.pdf"))
			{
		...
		if(correspondence!=null && correspondence.getOriginalFilename().length()>0)
		{
			if(!correspondence.getOriginalFilename().equals(law.getWritPatitionNumber()+"R.pdf"))
			{
		...
		if(noteSheet!=null && noteSheet.getOriginalFilename().trim().length()>0)
		{
			new File(law.getLocation()+law.getWritPatitionNumber()+"L.pdf").renameTo(new File("C:/Resources/"+law.getWritPatitionNumber()+"L.pdf"));
			System.out.println(keys.getRepository() + law.getWritPatitionNumber() + "L.pdf");
			System.out.println(noteSheet.getBytes());
			Files.write(Paths.get(keys.getRepository()+law.getWritPatitionNumber()+"L.pdf"),noteSheet.getBytes());
			System.out.println("C:/Resources/" + law.getWritPatitionNumber() + "L.pdf");
			System.out.println(keys.getRepository() + law.getWritPatitionNumber() + "L.pdf");
			System.out.println(law.getLocation() +law.getWritPatitionNumber() + "L.pdf");
			FileUtils.mergeFiles("C:/Resources/"+law.getWritPatitionNumber()+"L.pdf",keys.getRepository()+law.getWritPatitionNumber()+"L.pdf",law.getLocation()+law.getWritPatitionNumber()+"L.pdf");
			new File("C:/Resources/"+law.getWritPatitionNumber()+"L.pdf").delete();
			new File(keys.getRepository()+law.getWritPatitionNumber()+"L.pdf").delete();
			// update page count
		   law.setNoOfNoteeing(getPdfPageCount(law.getLocation() + law.getWritPatitionNumber() + "L.pdf"));
		}
		if(correspondence!=null && correspondence.getOriginalFilename().trim().length()>0)
		{
			new File(law.getLocation()+law.getWritPatitionNumber()+"R.pdf").renameTo(new File("C:/Resources/"+law.getWritPatitionNumber()+"R.pdf"));
			Files.write(Paths.get(keys.getRepository()+law.getWritPatitionNumber()+"R.pdf"),correspondence.getBytes());
			FileUtils.mergeFiles("C:/Resources/"+law.getWritPatitionNumber()+"R.pdf",keys.getRepository()+law.getWritPatitionNumber()+"R.pdf",law.getLocation()+law.getWritPatitionNumber()+"R.pdf");
			new File("C:/Resources/"+law.getWritPatitionNumber()+"R.pdf").delete();
			new File(keys.getRepository()+law.getWritPatitionNumber()+"R.pdf").delete();
			
			// update page count
		    law.setNoOfCoss(getPdfPageCount(law.getLocation() + law.getWritPatitionNumber() + "R.pdf"));
		}
		
		// update total pages
		int totalPage = Integer.parseInt(law.getNoOfNoteeing()) + Integer.parseInt(law.getNoOfCoss());
		law.setTotalPages(Integer.toString(totalPage));
		
		emlService.insertOrUpdateLawRecord(law);
		flashAttributes.addFlashAttribute("msg","File has been updated successfully.");
		commonService.insertLogs(uId,"Updated File of Law with Id:"+law.getWritPatitionNumber()+".");
		return "redirect:/updateFile?department=Law&sno="+law.getSno();
	}
	
	private String getPdfPageCount(String pdfPath) {
