...
public class UserController {
	...
	public String authUser(ModelMap model, HttpServletRequest request, @RequestParam("userId") String userId,
	...
	public String addUser(HttpServletRequest request, @ModelAttribute("userForm") Users user,
			@RequestParam("department") String[] departments, RedirectAttributes flashAttributes) {
		...
		if (userService.isUserIdExists(user.getUserId()))
			...
			if (user.getUserType().equals("Administrator")) {
				ArrayList<UserDepartments> userDepartments = new ArrayList<>();
				for (String department : departments) {
					if (department.contains("Abadi") || department.contains("Abadi 6%"))
						department = "Abadi 6%";

					if (department.contains("EAnldM"))
			...
			if (user.getUserType().equals("User")) {
				...
				for (String sector : sectors) {
					if (!sector.equals("") && sector.length() > 0 && sector.indexOf("(") != -1
							&& sector.indexOf(")") != -1) {
						UserDepartments userDepartment = new UserDepartments();
						System.out.println("Sector is " + sector);
						userDepartment
								.setDepartment(sector.substring(sector.lastIndexOf("(") + 1, sector.lastIndexOf(")")));
						userDepartment.setUserId(user.getUserId());
						userDepartments.add(userDepartment);
					}
				}
	...
	public String userDetails(ModelMap model, HttpServletRequest request, @RequestParam("userId") String userId) {
		...
		ArrayList<String> unSelectedDepartments = new ArrayList<>();
