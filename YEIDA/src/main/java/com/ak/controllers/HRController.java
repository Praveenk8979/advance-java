...
public class HRController {
	...
	public String updateHR(HttpServletRequest request, @ModelAttribute("HRForm") HR hr,
			@RequestParam("noteSheet") MultipartFile noteSheet,
			@RequestParam("correspondence") MultipartFile correspondence, RedirectAttributes flashAttributes)
			throws IOException {
		...
		if (noteSheet != null && noteSheet.getOriginalFilename().trim().length() > 0) {
			new File(hr.getLocation() + hr.getFileCode() + "L.pdf")
					.renameTo(new File("C:/Resources/" + hr.getFileCode() + "L.pdf"));
			...
			FileUtils.mergeFiles("C:/Resources/" + hr.getFileCode() + "L.pdf",
					keys.getRepository() + hr.getFileCode() + "L.pdf", hr.getLocation() + hr.getFileCode() + "L.pdf");
			new File("C:/Resources/" + hr.getFileCode() + "L.pdf").delete();
			new File(keys.getRepository() + hr.getFileCode() + "L.pdf").delete();

			// update page count
			hr.setNoOfNoteeing(getPdfPageCount(hr.getLocation() + hr.getFileCode() + "L.pdf"));
		}
		if (correspondence != null && correspondence.getOriginalFilename().trim().length() > 0) {
			new File(hr.getLocation() + hr.getFileCode() + "R.pdf")
					.renameTo(new File("C:/Resources/" + hr.getFileCode() + "R.pdf"));
			Files.write(Paths.get(keys.getRepository() + hr.getFileCode() + "R.pdf"), correspondence.getBytes());
			FileUtils.mergeFiles("C:/Resources/" + hr.getFileCode() + "R.pdf",
					keys.getRepository() + hr.getFileCode() + "R.pdf", hr.getLocation() + hr.getFileCode() + "R.pdf");
			new File("C:/Resources/" + hr.getFileCode() + "R.pdf").delete();
			new File(keys.getRepository() + hr.getFileCode() + "R.pdf").delete();

			// update page count
			hr.setNoOfCoss(getPdfPageCount(hr.getLocation() + hr.getFileCode() + "R.pdf"));
		}

		// update total pages
		int totalPage = Integer.parseInt(hr.getNoOfNoteeing()) + Integer.parseInt(hr.getNoOfCoss());
