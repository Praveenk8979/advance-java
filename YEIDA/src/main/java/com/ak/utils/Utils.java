...
public class Utils {
	...
	public String[] getHeader(String department) {
		if (department.equals("Residential") || department.equals("Abadi 6%") || department.equals("Institutional")
				|| department.equals("IT") || department.equals("Group Housing") || department.equals("Builder")
		...
		else if (department.equals("EM") || department.equals("EM3") || department.equals("HortiCulture")
		...
		else if (department.equals("Finance_Direct_Salary") || department.equals("Finance_Deputation_Salary")
				|| department.equals("Finance_tax") || department.equals("Finance_Labor_Cass")
				|| department.equals("Finance_Costing") || department.equals("Finance_tax"))
	...
	public ArrayList<String> generateHRParams(HR hr) {
		...
		if (hr.getDate() != null && hr.getDate().trim().length() > 0)
			params.add(hr.getDate() + "@date");

		if (hr.getYear() != null && hr.getYear().trim().length() > 0)
			params.add(hr.getYear() + "@year");
		...
		if (hr.getFileSubject() != null && hr.getFileSubject().trim().length() > 0)
			params.add(hr.getFileSubject() + "@fileSubject");

		if (hr.getEmployeeCode() != null && hr.getEmployeeCode().trim().length() > 0)
			params.add(hr.getEmployeeCode() + "@employeeCode");

		if (hr.getEmployeeName() != null && hr.getEmployeeName().trim().length() > 0)
			params.add(hr.getEmployeeName() + "@employeeName");

		if (hr.getEmployeeCategory() != null && hr.getEmployeeCategory().trim().length() > 0)
			params.add(hr.getEmployeeCategory() + "@employeeCategory");

		if (hr.getEmployeeDesignation() != null && hr.getEmployeeDesignation().trim().length() > 0)
			params.add(hr.getEmployeeDesignation() + "@employeeDesignation");

		if (hr.getEmployeeDob() != null && hr.getEmployeeDob().trim().length() > 0)
			params.add(hr.getEmployeeDob() + "@employeeDob");

		if (hr.getEmployeeDoj() != null && hr.getEmployeeDoj().trim().length() > 0)
			params.add(hr.getEmployeeDoj() + "@employeeDoj");
	...
	public ArrayList<String> generateLawParams(Law law) {
		...
		if (law.getDepartment() != null && law.getDepartment().trim().length() > 0)
			params.add(law.getDepartment() + "@department");
		
		if (law.getCourtName() != null && law.getCourtName().trim().length() > 0)
			params.add(law.getCourtName() + "@courtName");
			
		if (law.getWritPatitionNumber() != null && law.getWritPatitionNumber().trim().length() > 0)
			params.add(law.getWritPatitionNumber() + "@writPatitionNumber");
			
		if (law.getYear() != null && law.getYear().trim().length() > 0)
			params.add(law.getYear() + "@year");
			
			
		if (law.getSubject() != null && law.getSubject().trim().length() > 0)
			params.add(law.getSubject() + "@subject");
			
			
		if (law.getAdvocateName() != null && law.getAdvocateName().trim().length() > 0)
			params.add(law.getAdvocateName() + "@advocateName");
		...
		if(law.getSector() != null && law.getSector().trim().length()>0)
			params.add(law.getSector() + "@sector");
		
		if(law.getOpaFTS() != null && law.getOpaFTS().trim().length()>0)
			params.add(law.getOpaFTS() + "@opaFTS");
	...
	public String getAgendaLocation(String agendaNo, String fileNo) {
		...
		System.out.println(
				"Returning is " + file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(fileNo)));
