...
public class FileUtils {
			...
			if (uId != null && uId.indexOf(",") != -1) {
				if (agenda) {
					...
					for (PDDocument page : documents) {
						...
						if (name.indexOf("L.pdf") != -1)
							...
							System.out.println(
									webLocation + name.substring(0, name.indexOf(".pdf")) + "@" + count + ".pdf");
						}
						page.close();
						if (name.indexOf("L.pdf") != -1)
							addWaterMark(webLocation, webLocation,
									name.substring(0, name.indexOf(".pdf")) + "@" + count + "L.pdf", uId, true);
						else if (name.indexOf("R.pdf") != -1)
							addWaterMark(webLocation, webLocation,
									name.substring(0, name.indexOf(".pdf")) + "@" + count + "R.pdf", uId, true);
						else
							addWaterMark(webLocation, webLocation,
									name.substring(0, name.indexOf(".pdf")) + "@" + count + ".pdf", uId, true);
						count++;
					}
					document.close();
				}
				...
				if (true) {
					if (subdepartment == null)
						mergeFiles(fileLocation + name + "L.pdf", fileLocation + name + "R.pdf",
								"C:/Resources/" + name + ".pdf");
					else
	...
	public static void downloadFile(HttpServletResponse response, String name, String fileLocation, boolean merger,
			String webLocation, String id) {
		if (merger) {
	...
	public static void newDownloadFile(HttpServletResponse response, String name, String fileLocation, boolean merger,
			String webLocation, String id) {
		String[] fileName = name.split("\\.");
		if (merger) {
			System.out.println("-----------" + fileLocation + fileName[0] + "L.pdf");
			mergeFiles(fileLocation + fileName[0] + "L.pdf", fileLocation + fileName[0] + "R.pdf",
					"C:/Resources/" + fileName[0] + ".pdf");
			fileLocation = "C:/Resources/";
			name = fileName[0] + ".pdf";
		}
		File file = null;
	...
	private static String addWaterMark(String location, String webLocation, String file, String id, boolean view)
			throws Exception {
		...
		if (view) {
			pdfWriter = PdfWriter.getInstance(document,
					new FileOutputStream(webLocation + file.substring(0, file.indexOf(".pdf")) + "@print.pdf"));
			id = id.split(",")[0];
		} else
	...
	public static void generateGenReport(ArrayList<General> records, String location, HttpServletResponse response) {
		...
		titleCell.setCellValue("Yamuna Expressway Industrial Development Authority");
		...
		for (General record : records) {
			...
			r++;
			row.createCell(r).setCellValue(record.getSector());
			r++;
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getScheme());
			r++;
			row.createCell(r).setCellValue(record.getAllotmentNo());
			r++;
			row.createCell(r).setCellValue(record.getAlloteeName());
			r++;
			row.createCell(r).setCellValue(record.getPlotNo());
			r++;
			row.createCell(r).setCellValue(record.getPlotSize());
			r++;
			row.createCell(r).setCellValue(record.getBlock());
			r++;
			row.createCell(r).setCellValue(record.getMasterPlot());
			r++;
			row.createCell(r).setCellValue(record.getFts());
			r++;
			row.createCell(r).setCellValue(record.getOfficeOrder());
			r++;
			row.createCell(r).setCellValue(record.getOrderDate());
			r++;
			row.createCell(r).setCellValue(record.getAgenda());
			r++;
			row.createCell(r).setCellValue(record.getBoardMeeting());
			r++;
			row.createCell(r).setCellValue(record.getMeetingDate());
			i++;
		}
		try {
	...
	public static void generateFinReport(ArrayList<Finance> records, String location, HttpServletResponse response) {
		...
		titleCell.setCellValue(
				"Software generated report of Finance(" + records.get(0).getSubdepartment() + ") Department.");
		HSSFRow rowHead = sheet.createRow(2);
		...
		if (subd.equals("Bank Statement") || subd.equals("Loan")) {
			...
			for (Finance record : records) {
				...
				row.createCell(r).setCellValue(i - 2);
				r++;
				row.createCell(r).setCellValue(record.getBankName());
				r++;
				row.createCell(r).setCellValue(record.getBranchName());
				r++;
				row.createCell(r).setCellValue(record.getFileNo());
				r++;
				row.createCell(r).setCellValue(record.getStatement());
				r++;
				row.createCell(r).setCellValue(record.getPeriodOfYear());
				r++;
				row.createCell(r).setCellValue(record.getPatrawaliSankya());
				r++;
				row.createCell(r).setCellValue(record.getSubject());
				r++;
				row.createCell(r).setCellValue(record.getClerkName());
				r++;
				row.createCell(r).setCellValue(record.getRegisterName());
				r++;
				row.createCell(r).setCellValue(record.getSubdepartment());
				r++;
				row.createCell(r).setCellValue(record.getAccountNo());
				i++;
			}
		} else {
			HSSFCell cell = rowHead.createCell(0);
			...
			for (Finance record : records) {
				...
				row.createCell(r).setCellValue(record.getFileNo());
				r++;
				row.createCell(r).setCellValue(record.getCodeNo());
				r++;
				row.createCell(r).setCellValue(record.getYear());
				r++;
				row.createCell(r).setCellValue(record.getSubject());
				r++;
				row.createCell(r).setCellValue(record.getDesignation());
				r++;
				row.createCell(r).setCellValue(record.getFileType());
				r++;
				row.createCell(r).setCellValue(record.getSubdepartment());
				i++;
			}
	...
	public static void generateProReport(ArrayList<Project> records, String location, HttpServletResponse response) {
		...
		for (Project record : records) {
			...
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getWorkName());
			r++;
			row.createCell(r).setCellValue(record.getContractorName());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getCategory());
			r++;
			row.createCell(r).setCellValue(record.getOpaFts());
			r++;
			row.createCell(r).setCellValue(record.getSector());
			r++;
			row.createCell(r).setCellValue(record.getWorkCircle());
			r++;
			row.createCell(r).setCellValue(record.getFileNo());
			r++;
			row.createCell(r).setCellValue(record.getUploadDate());
			i++;
		}
		try {
			FileOutputStream out = new FileOutputStream(location + "Report.xls");
	...
	public static void generatePlanReport(ArrayList<Planning> records, String location, HttpServletResponse response) {
		...
		titleCell.setCellStyle(titleStyle);
		titleCell.setCellValue(
				"Software generated report of Planning (" + records.get(0).getSubDepartment() + ") Department");
		HSSFRow rowHead = sheet.createRow(2);
		String subd = records.get(0).getSubDepartment();
		if (subd.equals("Industry") || subd.equals("Institutional")) {
			...
			for (Planning record : records) {
				...
				row.createCell(r).setCellValue(record.getAllotmentNo());
				r++;
				row.createCell(r).setCellValue(record.getBpNo());
				r++;
				row.createCell(r).setCellValue(record.getFileType());
				r++;
				row.createCell(r).setCellValue(record.getApplicantName());
				r++;
				row.createCell(r).setCellValue(record.getPlotSize());
				r++;
				row.createCell(r).setCellValue(record.getPlotNo());
				r++;
				row.createCell(r).setCellValue(record.getBlockNo());
				r++;
				row.createCell(r).setCellValue(record.getSector());
				r++;
				row.createCell(r).setCellValue(record.getCategory());
				...
				row.createCell(r).setCellValue(record.getSubDepartment());
				r++;
				row.createCell(r).setCellValue(record.getClerk_Name());
				i++;
			}
		} else if (subd.equals("Building NOC")) {
			HSSFCell cell = rowHead.createCell(0);
			cell.setCellValue("S.No");
			cell.setCellStyle(style);
			cell = rowHead.createCell(1);
			...
			for (Planning record : records) {
				...
				row.createCell(r).setCellValue(record.getAllotmentNo());
				r++;
				row.createCell(r).setCellValue(record.getBn_no());
				r++;
				row.createCell(r).setCellValue(record.getFileType());
				r++;
				row.createCell(r).setCellValue(record.getApplicantName());
	...
	public static void generateLawReport(ArrayList<Law> records, String location, HttpServletResponse response) {
		...
		HSSFRow rowHead = sheet.createRow(2);
		HSSFCell cell = rowHead.createCell(cellCount++);
		cell.setCellValue("S.No");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("Department");
		cell.setCellStyle(style);
		
		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("Court Name");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("Writ Patition Number");
		cell.setCellStyle(style);
		...
		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("Advocate Name");
		cell.setCellStyle(style);
		...
		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("FTS/OPA NO");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("No_Of_Noteeing");
		cell.setCellStyle(style);
		
		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("Nop_Of_Coss");
		cell.setCellStyle(style);
		...
		int r = 0, i = 3;
		for (Law record : records) {
			r = 0;
			HSSFRow row = sheet.createRow(i++);
			row.createCell(r++).setCellValue(i - 3);
			row.createCell(r++).setCellValue(record.getDepartment());
			row.createCell(r++).setCellValue(record.getCourtName());
			row.createCell(r++).setCellValue(record.getWritPatitionNumber());
			row.createCell(r++).setCellValue(record.getYear());
			row.createCell(r++).setCellValue(record.getSubject());
			row.createCell(r++).setCellValue(record.getAdvocateName());
			row.createCell(r++).setCellValue(record.getRelatedDepartment());
			row.createCell(r++).setCellValue(record.getSector());
			row.createCell(r++).setCellValue(record.getOpaFTS());

			row.createCell(r++).setCellValue(record.getNoOfNoteeing());
			row.createCell(r++).setCellValue(record.getNoOfCoss());
			row.createCell(r++).setCellValue(record.getTotalPages());
	...
	public static void generateSystemReport(ArrayList<Systems> records, String location, HttpServletResponse response) {
		...
		int r = 0, i = 3;
		for (Systems record : records) {
			r = 0;
			HSSFRow row = sheet.createRow(i);
			row.createCell(r).setCellValue(i - 2);
			r++;
			row.createCell(r).setCellValue(record.getDate());
			r++;
			row.createCell(r).setCellValue(record.getLotNo());
			r++;
			row.createCell(r).setCellValue(record.getClerk_Name());
			r++;
			row.createCell(r).setCellValue(record.getFile_No());
			r++;
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getOptFts());
			r++;

			row.createCell(r).setCellValue(record.getSubjectName());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_NoteSheet());
			r++;

			row.createCell(r).setCellValue(record.getNo_Of_Cros());
			r++;
			 ...
			 */
			row.createCell(r).setCellValue(record.getOpt_Name());
			r++;
			row.createCell(r).setCellValue(record.getTotal_No_Of_Pages());
			i++;
	...
	public static void generateEM3Report(ArrayList<EM3> records, String location, HttpServletResponse response) {
		...
		for (EM3 record : records) {
			...
			row.createCell(r).setCellValue(record.getCategory());
			r++;
			row.createCell(r).setCellValue(record.getOpa_Fts());
			r++;
			row.createCell(r).setCellValue(record.getWorkName());
			r++;
			row.createCell(r).setCellValue(record.getContractorName());
	...
	public static void generateLandReport(ArrayList<Land> records, String location, HttpServletResponse response) {
		...
		for (Land record : records) {
			...
			row.createCell(r).setCellValue(record.getFileNo());
			r++;
			row.createCell(r).setCellValue(record.getFilesub());
			r++;
			row.createCell(r).setCellValue(record.getFileType());
			r++;
			row.createCell(r).setCellValue(record.getLine());
			r++;

			row.createCell(r).setCellValue(record.getOpaFts());
			r++;
			row.createCell(r).setCellValue(record.getVillage());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;

			row.createCell(r).setCellValue(record.getNo_of_notsheet());
			r++;
			row.createCell(r).setCellValue(record.getNo_of_cos());
			r++;
			row.createCell(r).setCellValue(record.getNo_of_a3());
			r++;
			row.createCell(r).setCellValue(record.getNo_of_a2());
			r++;
			row.createCell(r).setCellValue(record.getNo_of_a1());
			r++;
			row.createCell(r).setCellValue(record.getNo_of_a0());
			r++;
			row.createCell(r).setCellValue(record.getTotal_pages());
			r++;
			row.createCell(r).setCellValue(record.getServey_letter());
			r++;
			row.createCell(r).setCellValue(record.getNotifection());
			r++;
			row.createCell(r).setCellValue(record.getMorgeg_letter());
			r++;
			row.createCell(r).setCellValue(record.getMap_11());
			i++;
	...
	/*----------------------Marketing -----------------------------------*/
	public static void generateMarketingReport(ArrayList<Marketing> records, String location,
		...
		for (Marketing record : records) {
			...
			row.createCell(r).setCellValue(record.getDate());
			i++;
			row.createCell(r).setCellValue(record.getLot_No());
			r++;
			row.createCell(r).setCellValue(record.getClerk_Name());
			r++;
			row.createCell(r).setCellValue(record.getFile_No());
			...
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getFts_No_Opa_No());
			r++;

			row.createCell(r).setCellValue(record.getApplicant_Name());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_NoteSheet());
	...
	public static void generateHealthReport(ArrayList<Health> records, String location, HttpServletResponse response) {
		...
		for (Health record : records) {
			...
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getContrName());
			r++;
			row.createCell(r).setCellValue(record.getScheme());
			i++;
	...
	public static void generateHRReport(ArrayList<HR> records, String location, HttpServletResponse response) {
		...
		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("FILE SUBJECT");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES CODE");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES NAME");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES CATEGORY");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES DESIGNATION");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES DOB");
		cell.setCellStyle(style);

		cell = rowHead.createCell(cellCount++);
		cell.setCellValue("EMPLOYEES DOJ");
		cell.setCellStyle(style);
		...
		int r = 0, i = 3;
		for (HR record : records) {
			r = 0;
			HSSFRow row = sheet.createRow(i++);
			row.createCell(r++).setCellValue(i - 3);
			row.createCell(r++).setCellValue(record.getDate());
			row.createCell(r++).setCellValue(record.getYear());
			row.createCell(r++).setCellValue(record.getFileCode());
			row.createCell(r++).setCellValue(record.getFileNo());
			row.createCell(r++).setCellValue(record.getFileSubject());
			row.createCell(r++).setCellValue(record.getEmployeeCode());
			row.createCell(r++).setCellValue(record.getEmployeeName());
			row.createCell(r++).setCellValue(record.getEmployeeCategory());
			row.createCell(r++).setCellValue(record.getEmployeeDesignation());
			row.createCell(r++).setCellValue(record.getEmployeeDob());
			row.createCell(r++).setCellValue(record.getEmployeeDoj());
			row.createCell(r++).setCellValue(record.getOpaFts());
	...
	public static void generatePlanning2Report(ArrayList<Planning2> records, String location,
			HttpServletResponse response) {
		HSSFWorkbook workBook = new HSSFWorkbook();
		HSSFSheet sheet = workBook.createSheet("Records Report");
		HSSFFont titleFont = workBook.createFont();
		...
		for (Planning2 record : records) {
			...
			row.createCell(r).setCellValue(record.getClerk_Name());
			r++;
			row.createCell(r).setCellValue(record.getTypes_Of_Files());
			r++;
			row.createCell(r).setCellValue(record.getCategory());
			r++;
			row.createCell(r).setCellValue(record.getBP_No());
			r++;
			row.createCell(r).setCellValue(record.getAllotmentNo());
			r++;
			row.createCell(r).setCellValue(record.getApplicantName());
			r++;
			row.createCell(r).setCellValue(record.getPlotNo_HouseNo());
			r++;
			row.createCell(r).setCellValue(record.getPlot_Size());
			r++;
			row.createCell(r).setCellValue(record.getBlock_No());

			r++;
			row.createCell(r).setCellValue(record.getSecotr());
			r++;
			row.createCell(r).setCellValue(record.getOpaFts());
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_NoteSheet());
	...
	public static void generateEM2Report(ArrayList<EM2> records, String location, HttpServletResponse response) {
		...
		for (EM2 record : records) {
			...
			row.createCell(r).setCellValue(record.getOpaFts());
			r++;
			row.createCell(r).setCellValue(record.getName_Of_Work());
			r++;
			row.createCell(r).setCellValue(record.getContractor_Name());
			r++;
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getFile_number());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_NoteSheet());
			...
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_A3_PAGES());
			r++;
			row.createCell(r).setCellValue(record.getNO_OF_A2_PAGES());
			r++;
			row.createCell(r).setCellValue(record.getNO_OF_A1_PAGES());
			r++;
			row.createCell(r).setCellValue(record.getNO_OF_A0_PAGES());
			r++;
			row.createCell(r).setCellValue(record.getTotal_No_Of_Pages());
	...
	public static void generateEM3Report(String department, ArrayList<EM3> records, String location,
			HttpServletResponse response) {
		HSSFWorkbook workBook = new HSSFWorkbook();
		HSSFSheet sheet = workBook.createSheet("Records Report");
		HSSFFont titleFont = workBook.createFont();
		...
		for (EM3 record : records) {
			...
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getNoOfNoteSheet());
			r++;
			row.createCell(r).setCellValue(record.getNoOfCros());

			r++;
			row.createCell(r).setCellValue(record.getNoOfA0Pages());
			r++;
			row.createCell(r).setCellValue(record.getNoOfA1Pages());
			r++;
			row.createCell(r).setCellValue(record.getNoOfA2Pages());
			r++;
			row.createCell(r).setCellValue(record.getNoOfA3Pages());
			r++;
			row.createCell(r).setCellValue(record.getTotalNoOfPages());
	...
	public static void generateProjectTechReport(ArrayList<ProjectTech> records, String location,
			HttpServletResponse response) {
		HSSFWorkbook workBook = new HSSFWorkbook();
		HSSFSheet sheet = workBook.createSheet("Records Report");
		HSSFFont titleFont = workBook.createFont();
		...
		int r = 0, i = 3;
		for (ProjectTech record : records) {
			r = 0;
			HSSFRow row = sheet.createRow(i);
			row.createCell(r).setCellValue(i - 2);
			r++;
			row.createCell(r).setCellValue(record.getLine_No());
			r++;
			row.createCell(r).setCellValue(record.getDate());
			r++;
			row.createCell(r).setCellValue(record.getSector());
			...
			row.createCell(r).setCellValue(record.getDepartment());
			r++;
			row.createCell(r).setCellValue(record.getFileNumber());
			r++;
			row.createCell(r).setCellValue(record.getYear());
			r++;
			row.createCell(r).setCellValue(record.getNo_Of_NoteSheet());
